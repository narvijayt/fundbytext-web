...
// Shared field tokens (Figma node 5556:22007 — 56px rows, 12px radius, Mist-Gray border).
const LABEL      = "text-[12px] font-black uppercase tracking-[1px] text-[#003060]";
const FIELD_BASE = "h-14 w-full rounded-[12px] border bg-white px-5 text-[16px] text-[#003060] placeholder:text-[#9aa7b8] transition-colors focus:outline-none";
...
function EditIcon() {
    return (
        <svg className="h-5 w-5" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth={1.25} strokeLinecap="round" strokeLinejoin="round">
    ...
    return (
        <div className="flex flex-col gap-3">
            ...
            <button
                type="button"
                onClick={onEdit}
                className="group flex h-14 w-full items-center justify-between gap-3 rounded-[12px] border border-[#d4dee7] bg-white px-5 text-left transition-colors hover:border-[#0268c0]"
            >
                <span className={`truncate text-[16px] ${empty ? "text-[#aeb5bd]" : "text-[#7e8a96]"}`}>{value}</span>
                <span className="flex shrink-0 items-center gap-2 text-[14px] text-[#4b5563] transition-colors group-hover:text-[#0268c0]">
                    <EditIcon />
                    Edit
                </span>
            </button>
    ...
    return createPortal(
        ...
        >
            <div
                ...
                className={`flex max-h-[calc(100vh-2rem)] w-full max-w-[600px] flex-col overflow-hidden rounded-[16px] bg-white shadow-[0px_16px_32px_0px_rgba(15,29,67,0.24)] transition-transform duration-200 motion-reduce:transition-none ${shown ? "scale-100" : "scale-95"}`}
            >
                {/* Header */}
                <div className="flex shrink-0 items-center justify-between gap-3 border-b border-[#e8eaee] bg-[#0278de] py-5 pl-8 pr-4 text-white">
                    <h2 id="edit-profile-title" className="text-[20px] font-bold leading-[1.15]">Edit Profile</h2>
                    <button onClick={close} aria-label="Close" className="-mr-1 flex h-9 w-9 items-center justify-center rounded-lg text-white/85 transition-colors hover:bg-white/15 hover:text-white active:bg-white/25">
                        <svg className="h-[18px] w-[18px]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round"><path d="M6 6l12 12M18 6L6 18" /></svg>
                    </button>
                </div>
        ...
        <form onSubmit={(e) => { e.preventDefault(); handleSave(); }} className="flex flex-1 flex-col overflow-hidden">
            <div className="flex flex-1 flex-col gap-8 overflow-y-auto p-8">
                ...
                <div className="flex items-center gap-6">
                    <div className="flex h-20 w-20 shrink-0 items-center justify-center overflow-hidden rounded-full bg-[#e7e9eb]">
                        {photoSrc && !imgError ? (
                            <Image src={photoSrc} alt={displayName} width={80} height={80} className="h-full w-full object-cover" onError={() => setImgError(true)} />
                        ) : (
                            <span className="text-2xl font-bold text-[#7e8a96]">{initial}</span>
                        )}
                    </div>
                    <div className="flex flex-col items-start gap-3">
                        <p className="text-[18px] font-medium leading-[1.4] text-[#003060]">Change Profile Picture</p>
                        <button
                            type="button"
                            onClick={() => fileInputRef.current?.click()}
                            className="inline-flex h-[30px] items-center justify-center rounded-full bg-gradient-to-b from-[#ea6725] to-[#ff8c53] px-6 text-[12px] font-medium tracking-[0.25px] text-white shadow-[0px_8px_15px_-8px_#ea6725,0px_12px_60px_-12px_rgba(255,140,83,0.4)] transition-[filter] hover:brightness-105"
                        >
                ...
                <div className="flex flex-col gap-6">
                        ...
                        <>
                            ...
                            {emailChanged && (
                                <div className="flex flex-col gap-3">
                                    ...
                                    {errors.confirm_email && <p className="-mt-1.5 text-xs text-red-500">{errors.confirm_email}</p>}
                                </div>
                            )}
                        </>
                    ) : (
                    ...
                    {editing.phone ? (
                        <div className="flex flex-col gap-3">
                            ...
                            <div className="flex h-14 items-center rounded-[12px] border border-[#d4dee7] bg-white transition-colors focus-within:border-[#0268c0] focus-within:ring-2 focus-within:ring-[#0268c0]/15">
                                <span className="pl-5 pr-3 text-[16px] text-[#7e8a96]">+1</span>
                                <span className="h-6 w-px bg-[#d4dee7]" />
                                <input ref={phoneRef} type="tel" value={form.phone} onChange={(e) => set("phone", e.target.value)} placeholder="(214) 987-6543" className="h-full w-full rounded-r-[12px] bg-transparent px-4 text-[16px] text-[#003060] placeholder:text-[#9aa7b8] focus:outline-none" />
                            </div>
                        </div>
                    ) : (
            ...
            {/* Footer */}
            <div className="flex shrink-0 items-center justify-end gap-2 border-t border-[#e8eaee] py-4 pl-8 pr-5">
                <button type="button" onClick={onClose} disabled={saving} className="rounded-[10px] border border-[#dde0e3] px-3.5 pb-3 pt-2.5 text-[14px] font-medium leading-none tracking-[0.15px] text-[#003060] transition-colors hover:bg-gray-50 disabled:opacity-60">
                    Cancel
                </button>
                <button type="submit" disabled={saving} className="rounded-[10px] bg-[#0268c0] px-3.5 pb-3 pt-2.5 text-[14px] font-medium leading-none tracking-[0.15px] text-white transition-[filter] hover:brightness-110 disabled:opacity-60">
