...
export default function RemoveParticipantModal({ memberId, campaignSlug, name, raised, isSelf, onClose }: Props) {
    const router = useRouter();
    const [removing, setRemoving] = useState(false);
    const [error,    setError]    = useState<string | null>(null);
    const [shown,    setShown]    = useState(false);

    const blocked = raised > 0;
    ...
    return (
        ...
        >
            ...
            >
                <div className="p-6">
                    <div className={`mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-full ${blocked ? "bg-amber-100 text-amber-600" : "bg-red-100 text-red-600"}`}>
                        {blocked ? (
                    ...
                    {blocked ? (
                        ...
                        <>
                            <h2 className="text-center text-[16px] font-bold text-[#003060]">
                                {isSelf ? "Leave participant role?" : `Remove ${name}?`}
                            </h2>
                            <p className="mt-2 text-center text-[13px] leading-relaxed text-[#7e8a96]">
                                {isSelf
                                    ? "Your organizer role will be kept. This can't be undone."
                                    : "This participant will be removed from the campaign. This can't be undone."}
                            </p>

                            {error && (
                                <p role="alert" className="mt-4 rounded-lg bg-red-50 px-3.5 py-2.5 text-center text-[13px] font-medium text-red-600">{error}</p>
                            )}

                            <div className="mt-5 flex gap-3">
                                <button onClick={close} disabled={removing} className="flex-1 rounded-[10px] border border-[#d4dee7] py-2.5 text-[14px] font-semibold text-[#003060] transition-colors hover:bg-gray-50 disabled:opacity-60">
                                    Cancel
                                </button>
                                <button onClick={handleRemove} disabled={removing} className="flex-1 rounded-[10px] bg-red-500 py-2.5 text-[14px] font-semibold text-white transition-[filter] hover:brightness-105 disabled:opacity-60">
