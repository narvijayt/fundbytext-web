...
type Props = {
    storageKey:   string;        // unique per participant + campaign — persists the dismissal
    campaignName: string;
...
/* The FundBuddy onboarding welcome shown once to a participant. It's rendered
   only on the client and only when it hasn't been dismissed before (localStorage),
   so a returning participant never sees it again — and there's no server/client
   flash. Smoothly expands in and collapses out (grid-rows trick). Not sticky. */
export default function FundBuddyOnboarding({ storageKey, campaignName, donorTarget, launchLabel }: Props) {
    const [ready, setReady] = useState(false); // decided (client, not previously dismissed)
    const [open,  setOpen]  = useState(false);  // expanded / visible
    const [gone,  setGone]  = useState(false);

    useEffect(() => {
        let dismissed = false;
        try { dismissed = localStorage.getItem(storageKey) === "1"; } catch { /* ignore */ }
        if (dismissed) { setGone(true); return; }
        setReady(true);
    ...
    // Nothing on the server (ready=false) and nothing for a participant who already dismissed it.
    ...
    return (
        <div
            ...
            style={{ gridTemplateRows: open ? "1fr" : "0fr", opacity: open ? 1 : 0, marginTop: open ? undefined : 0 }}
        >
