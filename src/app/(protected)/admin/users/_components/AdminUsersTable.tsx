        ...
        <tr className="border-b border-[#eef1f4] last:border-0">
            <td className="py-4 pl-5 pr-4">
                <div className="flex items-center gap-3 animate-pulse">
                    ...
                    <div className="space-y-1.5"><Bar w="w-28" /><Bar w="w-16" /></div>
                </div>
            </td>
            <td className="px-4 py-4"><div className="animate-pulse space-y-1.5"><Bar w="w-36" /><Bar w="w-24" /></div></td>
            <td className="px-4 py-4"><div className="animate-pulse"><Bar w="w-8" /></div></td>
...
export default function AdminUsersTable(props: Props) {
    const [rows,     setRows]     = useState<AdminUserRow[]>(props.initialUsers);
    ...
    async function fetchPage(a: FetchArgs) {
        ...
        try {
            ...
            const data = await res.json().catch(() => null) as { users?: AdminUserRow[]; total?: number } | null;
            if (id !== reqId.current) return;
    ...
    return (
        <div ref={sectionRef} className="scroll-mt-6">
            {/* Header row: count + create */}
            <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
                <p className="text-[13px] text-[#9aa7b8]">{total.toLocaleString()} total</p>
            ...
            <div className="mb-5 flex flex-wrap items-center gap-3">
                ...
                <select value={sort} onChange={(e) => onSort(e.target.value)} className={SELECT_CLS} aria-label="Sort users">
                    ...
                    <table className="w-full min-w-[820px] text-sm">
                        <thead>
                            <tr className="bg-[#0268c0] text-white">
                                ...
                                <th className={`${TH_CLS} text-right`}>Campaigns</th>
                            ...
                            {loading ? (
                                ...
                                <tr>
                                    <td colSpan={6} className="px-5 py-14 text-center text-sm italic text-[#9aa7b8]">
                                        {search.trim() || filter !== "all" ? "No users match your filters." : "No users found."}
                                    </td>
                                </tr>
                            ) : (
                                rows.map((u) => (
                                    <tr key={u.id} className="border-b border-[#eef1f4] transition-colors last:border-0 hover:bg-[#f7f9fb]">
                                        <td className="py-3.5 pl-5 pr-4">
                                            <div className="flex items-center gap-3">
                                                {u.profile_photo_url ? (
                                                    // eslint-disable-next-line @next/next/no-img-element
                                                    <img src={u.profile_photo_url} alt="" className="h-8 w-8 shrink-0 rounded-full border border-[#e7e9eb] object-cover" />
                                                ) : (
                                                    <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[#eef2f7] text-xs font-bold uppercase text-[#7e8a96]">
                                                        {u.first_name[0]}{u.last_name[0]}
                                                    </div>
                                                )}
                                                <div className="min-w-0">
                                                    <p className="flex items-center gap-1.5 text-[13px] font-semibold text-[#003060]">
                                                        <span className="truncate">{u.first_name} {u.last_name}</span>
                                                        {u.role === "admin" && <span className="shrink-0 rounded bg-purple-100 px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wide text-purple-700">Admin</span>}
                                                        {u.id === props.currentUserId && <span className="shrink-0 rounded border border-[#0268c0]/20 bg-blue-50 px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wide text-[#0268c0]">You</span>}
                                                    </p>
                                                    {u.username && <p className="truncate text-xs font-medium text-[#0268c0]">@{u.username}</p>}
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-4 py-3.5 text-[#7e8a96]">
                                            ...
                                            <p className="mt-0.5 text-xs text-[#9aa7b8]">
                                                {u.last_sign_in_at ? `Last signed in ${fmtRelative(u.last_sign_in_at)}` : "Never logged in"}
                                            </p>
                                        </td>
                                        <td className="px-4 py-3.5 text-right text-[13px] text-[#7e8a96]">{u.campaigns_count}</td>
                                        <td className="whitespace-nowrap px-4 py-3.5 text-[13px] text-[#9aa7b8]">{fmtDate(u.created_at)}</td>
                                        <td className="px-4 py-3.5">
                                            {u.deleted_at ? (
                                                <span className="rounded-full bg-gray-100 px-2.5 py-1 text-[11px] font-semibold text-gray-500">Deleted</span>
                                            ) : u.is_suspended ? (
                                                <span className="rounded-full bg-red-100 px-2.5 py-1 text-[11px] font-semibold text-red-600">Suspended</span>
                                            ) : (
                                                <span className="rounded-full bg-green-100 px-2.5 py-1 text-[11px] font-semibold text-green-700">Active</span>
                                            )}
                                        </td>
                                        <td className="py-3.5 pl-4 pr-5 text-right">
                                            <Link href={`/admin/users/${u.id}`} className="text-[13px] font-semibold text-[#0268c0] hover:underline">View</Link>
                                        </td>
                                    </tr>
                                ))
                            )}
