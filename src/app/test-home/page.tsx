...
const PAY              = "/assets/marketing/footer";   // real payment-brand + social SVGs
...
// The FundByText "F" glyph (from logo-icon.svg) — reused for faint watermarks.
...
// Procedural film-grain noise via SVG feTurbulence — replaces the raster vid-noise.png.
...
// A faint, tiled FundByText "F" watermark (scattered + rotated) — coded SVG pattern.
...
// A regular grid of tiny dots — coded SVG, sized/coloured per call.
function dotsUri(rgba: string, gap = 20, r = 1.4) {
    const svg =
        `<svg xmlns='http://www.w3.org/2000/svg' width='${gap}' height='${gap}'>` +
        `<circle cx='${r}' cy='${r}' r='${r}' fill='${rgba}'/></svg>`;
    return `url("data:image/svg+xml,${encodeURIComponent(svg)}")`;
}
...
export default async function HomePage() {
    ...
    const heroCards = liveCampaigns.map((c) => ({
    ...
    type LiveCard = typeof heroCards[0];
    type StaticCard = typeof STATIC_CARDS[0];

    // Marketing hero always shows 5 cards (the design's staggered row): real
    // campaigns first, padded with the curated demo cards to fill out to five.
    const cards: (LiveCard | StaticCard)[] = [...heroCards, ...STATIC_CARDS].slice(0, 5);
        ...
        <div className="font-sans text-gray-900 overflow-x-hidden">
            ...
            <section className="relative overflow-hidden min-h-[680px] md:min-h-[820px] lg:min-h-[960px]">
                ...
                <div className="absolute inset-0 pointer-events-none overflow-hidden">
                    ...
                    {/* L4 — Noise grain (coded feTurbulence, not a raster image) */}
                    <div className="absolute inset-0 mix-blend-overlay opacity-[0.18] pointer-events-none"
                    ...
                    <div className="flex flex-col md:flex-row items-center gap-10 md:gap-8 lg:gap-16">
                        ...
                        <div className="flex relative flex-none w-full max-w-[340px] md:max-w-[300px] lg:w-[536px] lg:max-w-none items-center justify-center h-[580px] lg:h-[623px]">
                            <div className="absolute inset-0 overflow-hidden rounded-[32px]"
                                ...
                                {/* dot grid */}
                                <div className="absolute inset-0" style={{ backgroundImage: dotsUri("rgba(255,255,255,0.16)", 22, 1.5), backgroundRepeat: "repeat" }} />
                                {/* faint F watermark */}
                                <div className="absolute inset-0" style={{ backgroundImage: fWatermarkUri(0.05), backgroundRepeat: "repeat", backgroundSize: "210px 210px" }} />
                                {/* soft chevron sweep on the right */}
                                <div className="absolute -right-[18%] top-0 bottom-0 w-[78%] opacity-50"
                                    style={{ background: "linear-gradient(115deg,transparent 38%,rgba(255,255,255,0.10) 50%,transparent 62%)", clipPath: "polygon(35% 0,100% 0,100% 100%,35% 100%,72% 50%)" }} />
                            </div>
            ...
            <section className="relative overflow-hidden rounded-tl-[32px] rounded-tr-[32px] lg:rounded-tl-[64px] lg:rounded-tr-[64px] pt-10 md:pt-20 lg:pt-28 pb-10 lg:pb-20 px-4 sm:px-6 lg:px-[144px] flex flex-col items-center gap-8 lg:gap-12"
                ...
                <div className="absolute left-1/2 -translate-x-1/2 -top-40 w-[1400px] h-[700px] rounded-[50%] pointer-events-none"
                    style={{ background: "radial-gradient(ellipse at 50% 50%,rgba(255,255,255,0.9) 0%,rgba(255,255,255,0.5) 35%,transparent 70%)", filter: "blur(40px)" }} />
                ...
                <div className="max-w-[1632px] mx-auto px-4 sm:px-6 lg:px-[144px] mb-16 lg:mb-24">
                    <div className="flex flex-col lg:flex-row items-start justify-center gap-8 lg:gap-10">
                        ...
                        <div className="relative w-full lg:flex-none lg:w-[556px]">
                            ...
                            <div className="relative rounded-2xl overflow-hidden lg:mx-[19px] lg:mt-[19px]"
                                style={{ height: 302 }}>
                                {/* Photo ships with its leaderboard chip baked in (matches Figma) */}
                    ...
                    <div className="flex-1 bg-white rounded-[24px] p-7 sm:p-10 lg:p-12">
                        <div className="flex flex-col sm:flex-row gap-10 sm:gap-12 lg:gap-16">
                            ...
                            <div className="sm:w-[180px] flex flex-col">
                                ...
                                <p className="text-[#7e8a96] text-sm leading-relaxed mt-auto pt-10">1901 Thornridge Cir. Shiloh, Hawaii 81063</p>
                            </div>
                        </div>
                    </div>
