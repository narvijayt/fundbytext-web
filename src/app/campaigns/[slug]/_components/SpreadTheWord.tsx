    ...
    return (
        <div className="bg-white rounded-2xl shadow-sm p-6 space-y-6">
            ...
            <div className="flex items-center justify-between gap-4">
                <h2 className="text-xl font-extrabold text-gray-900">Spread the Word</h2>
            ...
            {/* Video placeholder */}
            <div
                className="relative aspect-video rounded-2xl overflow-hidden bg-linear-to-br from-gray-200 to-gray-300 cursor-pointer group"
            ...
            {/* Shareable images — portrait (9:16) + square + QR code */}
            <div className="flex gap-4 h-64 sm:h-80">
                {/* Portrait shareable (9:16) */}
                <div className="relative flex-5 rounded-xl overflow-hidden bg-gray-100 min-w-0">
                    {galleryUrls[0] ? (
                        <Image src={galleryUrls[0]} alt="Share image" fill className="object-cover" />
                    ) : (
                        <PlaceholderTile />
                    )}
                    <div className="absolute top-2 right-2 w-7 h-7 rounded-full bg-white/90 flex items-center justify-center shadow-sm cursor-pointer hover:bg-white transition-colors">
                ...
                {/* Square shareable (1:1) */}
                <div className="relative flex-7 rounded-xl overflow-hidden bg-gray-100 min-w-0">
                    {galleryUrls[1] ? (
                        <Image src={galleryUrls[1]} alt="Share image 2" fill className="object-cover" />
                    ) : (
                        <PlaceholderTile />
                    )}
                    <div className="absolute top-2 right-2 w-7 h-7 rounded-full bg-white/90 flex items-center justify-center shadow-sm cursor-pointer hover:bg-white transition-colors">
                ...
                {/* QR code */}
                <div className="relative flex-5 rounded-xl overflow-hidden bg-gray-50 border border-gray-200 flex flex-col items-center justify-center gap-2 min-w-0">
                    <QRIcon />
                    <p className="text-[10px] text-gray-400 font-medium text-center leading-tight px-2">Scan to donate</p>
                    <div className="absolute top-2 right-2 w-7 h-7 rounded-full bg-white/90 flex items-center justify-center shadow-sm cursor-pointer hover:bg-white transition-colors">
                        <svg className="w-3.5 h-3.5 text-gray-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                            <path strokeLinecap="round" strokeLinejoin="round" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" />
                        </svg>
            ...
            {/* FundBuddy speech bubble */}
            <div className="flex items-stretch gap-0">
                {/* Mascot — sits at the bottom */}
                <div className="shrink-0 flex items-end justify-center w-20 sm:w-28">
                    <Image
                        src="/assets/campaigns/ask-buddy.svg"
                        alt="FundBuddy"
                        width={96}
                        height={130}
                        className="w-16 h-22 sm:w-20 sm:h-28 object-contain object-bottom"
                    />
                </div>

                {/* Speech bubble */}
                <div
                    className="flex-1 rounded-3xl rounded-bl-none p-5 border border-orange-200"
                    style={{ background: "#FFF7F0" }}
                >
                    <p className="text-sm font-medium text-orange-900 leading-relaxed">
                        Hover on any of the shareables above to get content about what they try for!
                    </p>
                    <div className="mt-3 flex items-center gap-3 flex-wrap">
                        <a
                            href={`sms:?body=${encodeURIComponent(campaignUrl)}`}
                            className="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-white text-sm font-semibold transition-opacity hover:opacity-90 shadow-sm"
...
function QRIcon() {
    return (
        <svg viewBox="0 0 21 21" className="w-12 h-12 text-gray-700" fill="currentColor">
