    ...
    return (
        <div className="flex items-center">
            <div className="flex flex-col items-center">
                <div
                    className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${circleColor}`}
                >
                    {status === "done" ? "✓" : num}
                </div>
                <span className={`text-[10px] mt-1 uppercase tracking-wide ${labelColor}`}>
                    {label}
                </span>
            </div>
            {num < 5 && (
