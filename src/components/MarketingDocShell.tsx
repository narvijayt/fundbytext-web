...
/* ── Shared doc-page shell (privacy / terms / cookies / FAQ) ─────────────────
   The tuned marketing hero — badge + title + intro over the blue gradient, grey
   dot grid, sidebar grain and the deep white arch — with a white content card
   the caller fills. Extracted so every long-form page shares one hero rather
   than drifting copies (the overlap + blue height were tuned by hand). */
...
/* The doc badge: a white→blue-10 gradient pill with a 16px sentence-case navy
   label (not the marketing SectionBadge's 12px uppercase tracked one). */
    ...
    return (
        <div className="font-sans text-gray-900 overflow-x-hidden">
            ...
            {/* Blue block wraps the nav + headline so its height follows the content;
                the card is pulled up over it (blue height and overlap are independent
                — the blue is set by the block's bottom pad, the overlap by the card's
                negative top margin). */}
